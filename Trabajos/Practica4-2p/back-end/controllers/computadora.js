...
const updateComputadora = async (req, res = response) => {
  ...
  const computadoraUpdate = await Computadora.findByIdAndUpdate(
    id,
    data,
    { new: true }
  );
...
const deleteComputadora = async (req, res = response) => {
  const { id } = req.params;
  try {
    const deletedComputadora = await Computadora.findByIdAndDelete(id);
    if (!deletedComputadora) {
      return res.status(404).json({ msg: 'Computadora no encontrada' });
    }
    res.json({ msg: 'Computadora eliminada exitosamente' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: 'Error al eliminar la computadora' });
  }
};
